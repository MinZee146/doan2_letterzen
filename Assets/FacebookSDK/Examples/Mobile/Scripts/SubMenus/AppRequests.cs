...
namespace Facebook.Unity.Example
{
    ...
    internal class AppRequests : MenuBase
    {
        ...
        protected override void GetGui()
        {
            ...
            {
                var filter = new List<object>() { "app_users" };
            ...
            {
                var filter = new List<object>() { "app_non_users" };
                FB.AppRequest("Test Message", null, filter, null, 0, string.Empty, string.Empty, this.HandleResult);
            }
            ...
            {
                var action = this.GetSelectedOGActionType();
                if (action != null)
                {
                    FB.AppRequest(
                        this.requestMessage,
        ...
        {
            var actionString = this.actionTypeStrings[this.selectedAction];
