...
public class FBWindowsPurchaseManager : MonoBehaviour
{
    ...
    private void ProcessGetCatalog(ICatalogResult result)
    {
        ...
        if (FB.IsLoggedIn)
        {
            ...
            else
            {
                ...
                {
                    var itemInfo = item.Title + "\n";
                    itemInfo += item.Description + "\n";
                    itemInfo += item.Price;

                    var newProduct = Instantiate(ProductGameObject, CatalogPanelTarnsform);
    ...
    {
        var UserPicture = new Texture2D(32, 32);
    ...
    private void processPurchases(IPurchasesResult result)
    {
        ...
        if (FB.IsLoggedIn)
        {
            ...
            else
            {
                ...
                {
                    foreach (var item in result.Purchases)
                    {
                        var itemInfo = item.ProductID + "\n";
                        itemInfo += item.PurchasePlatform + "\n";
                        itemInfo += item.PurchaseToken;

                        var newProduct = Instantiate(ProductGameObject, PurchasesPanelTarnsform);
